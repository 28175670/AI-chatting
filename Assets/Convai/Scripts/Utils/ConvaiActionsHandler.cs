    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private IEnumerator DoAction(ConvaiAction action)
        {
            ...
            switch (action.Verb)
            {
                ...
                case ActionChoice.OpenWebsite:
                    // Call the Crouch function and yield until it's completed
                    OpenWebsite();
                    break;
      
                case ActionChoice.OpenMusic:
                    // Call the Crouch function and yield until it's completed
                    StartMusic();
                    break;

                case ActionChoice.StopMusic:
                    // Call the Crouch function and yield until it's completed
                    StopMusic();
                    break;
                case ActionChoice.Resumeplayback:
                    // Call the Crouch function and yield until it's completed
                    Resumeplayback();
                    break;
                case ActionChoice.Turnoffmusic:
                // Call the Crouch function and yield until it's completed
                    Turnoffmusic();
                    break;
            }
        ...
        {
            string urI = "https://www.youtube.com/shorts/sfH8iajpc5Y";
            Application.OpenURL(urI);
           
        }
        private void OpenWebsite2()
        {
            string urI2 = "https://zh.wikipedia.org/zh-tw/%E5%8A%89%E5%81%89%E5%81%A5";
            Application.OpenURL(urI2);
        }
        private void StartMusic()
        {
            Instantiate(_music, transform.position, transform.rotation);
            //GameObject[] musiccount = GameObject.FindGameObjectsWithTag("music");
            //int Mcount = musiccount.Length;
        }
        private void Resumeplayback()
        {
            GameObject _musicS;
            _musicS = GameObject.FindWithTag("music");
            _musicS.GetComponent<AudioSource>().Play();
        }
        private void StopMusic()
        {
            GameObject _musicS;
            _musicS = GameObject.FindWithTag("music");
            _musicS.GetComponent<AudioSource>().Stop();
        }
        private void Turnoffmusic()
        {
            GameObject _musicS;
            _musicS = GameObject.FindWithTag("music");
            _musicS.SetActive(false);
        }
